...
دکمه‌ها و… همهٔ ۴۴ پلاگین (۳۶ نسل قبل + ۷ پلاگین نسل هشتم) را تغذیه می‌کند
...
Pyrogram (تلگرام)  ──آپدیت──►  bot/adapter/pyrogram_app.py
   ...
   dispatcher ← host.emit     out_sink → send_message          action_sink/audit_sink
        │                                                          │
        ▼                                                          ▼
   44 پلاگین                                           delete/ban/mute/kick/promote/…
```
...
- **فرمان متنی** (`/ban …`، «بن …»، …) → `dispatcher.try_dispatch_command`:
  هر ۱۱۸ فرمان از ۴۴ پلاگین + `/help` پویا، `/start`، `/plugins`.
...
- **کلیک دکمه** (`callback`) → شش پیشوند ثبت‌شده:
