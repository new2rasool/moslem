...
- FFmpeg و ابزارهای سیستمی را نصب می‌کند. اگر ffmpeg روی سیستم نباشد، نسخهٔ
  استاتیک `imageio-ffmpeg` نصب و **روی PATH قرار می‌گیرد**:
  در لینوکس با symlink به `venv/bin/ffmpeg` و در ویندوز با کپی به
  `venv\Scripts\ffmpeg.exe`. مسیر باینری از خود
  `imageio_ffmpeg.get_ffmpeg_exe()` پرسیده می‌شود، نه hardcode.
...
> ⚠️ اگر یکی از مقادیر را خالی بگذارید، اسکریپت `.env` را با مقدار
> `PUT_YOUR_..._HERE` می‌نویسد و **بات را اجرا نمی‌کند**. هیچ‌گاه
> `.env.example` روی `.env` کپی نمی‌شود — یعنی بات هرگز با اعتبارنامهٔ
> شخص دیگری بالا نمی‌آید. فایل را کامل کنید و دوباره اجرا کنید.
...
خروجی باید `ALL TESTS PASSED` باشد (۷ فایل تست، ۱۷۹ بررسی).

تست‌ها **هیچ چیزی را در پوشهٔ پروژه نمی‌نویسند**: `tests/_bootstrap.py` قبل از
import شدن ماژول‌ها، مسیر `.env`، `database.sqlite`، `sessions/` و `downloads/`
را به یک پوشهٔ موقت منتقل می‌کند. پس اجرای تست روی سرورِ در حال کار، توکن بات
یا جدول‌های شارژ/نصب را پاک نمی‌کند.
...
- **FFmpeg حتماً لازم است** — اگر نصب نشود پخش صوتی/تصویری کار نمی‌کند.
  بررسی کنید با `ffmpeg -version` اجرا شود.
- اگر در لاگ شروع این پیام را دیدید:
  `[!] A helper session file exists but the helper is NOT online.`
  یعنی فایل `sessions/helper.session` وجود دارد ولی هلپر بالا نیامده
  (نشست منقضی/باطل). تا رفع آن، دستورات پخش، تیوی و ویس‌چت غیرفعال
  می‌مانند؛ فایل را پاک کنید و `/login` را دوباره انجام دهید.
- `.env` را هرگز commit نکنید (در `.gitignore` هست) و توکن لو رفته را
  با @BotFather ریست کنید.
